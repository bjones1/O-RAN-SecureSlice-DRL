...
bool malicious=0;
bool slicecheck=0;
int tx_threshold = 130000;

void ue_data_to_DRL(const std::map<std::string, long long int>& dictionary, std::string ue_name) {
    ...
    std::string jsonDict = "{\"" + ue_name + "\":{";
    for (auto it = dictionary.begin(); it != dictionary.end(); ++it) {
        if (it != dictionary.begin())
            jsonDict += ",";
        jsonDict += "\"" + it->first + "\":\"" + std::to_string(it->second) + "\"";
    }
    jsonDict += "}}";

    const char* message = jsonDict.c_str();

    send(new_socket, message, strlen(message), 0);

    char buffer[1024] = {0};
    int valread = read(new_socket, buffer, 1024);
    char* result = new char[1024 + 1];
    std::memcpy(result, buffer, 1024);
    result[1024] = '\0';
    mdclog_write(MDCLOG_DEBUG, "%s", result); 
...
bool App::handle(e2sm::kpm::KpmIndication *kind)
{
    ...
    for (auto it = report->ues.begin(); it != report->ues.end(); ++it) {
	    // create and send UE data
	    std::map<std::string, long long int> ue_data = {
	      {"dl_bytes", (long long int)it->second.dl_bytes},
	      {"ul_bytes", (long long int)it->second.ul_bytes},
	      {"dl_prbs", (long long int)it->second.dl_prbs},
	      {"ul_prbs", (long long int)it->second.ul_prbs},
	      {"tx_pkts", (long long int)it->second.tx_pkts},
	      {"tx_errors", (long long int)it->second.tx_errors},
	      {"tx_brate", (long long int)it->second.tx_brate},
	      {"rx_pkts", (long long int)it->second.rx_pkts},
	      {"rx_errors", (long long int)it->second.rx_errors},
	      {"rx_brate", (long long int)it->second.rx_brate},
	      {"dl_cqi", it->second.dl_cqi},
	      {"dl_ri", it->second.dl_ri},
	      {"dl_pmi", it->second.dl_pmi},
	      {"ul_phr", it->second.ul_phr},
	      {"ul_sinr", it->second.ul_sinr},
	      {"ul_mcs", it->second.ul_mcs},
	      {"ul_samples", (long long int)it->second.ul_samples},
	      {"dl_mcs", it->second.dl_mcs},
	      {"dl_samples", (long long int)it->second.dl_samples}
	    };
	    ue_data_to_DRL(ue_data, std::to_string(it->first).c_str());
	}
