...
def train(args):
    ...
    if args.model_type == "DQN":
        agent = DQN(state_size, action_size, seed=0, DDQN=False).to(device)
        ...
        agent = DDQN(state_size, action_size, seed=0, DDQN=True).to(device)
        ...
        agent = DQN_Dueling(state_size, action_size, seed=0, DDQN=True).to(device)
